...
export default function Enquiry() {
    ...
    useEffect(() => {
        axios.post(import.meta.env.VITE_API_URL + import.meta.env.VITE_ENQUIRY_VIEW,
            {
                name: search,
                page: currentPage,
                limit:10
            ...
            .then((response) => {
                if (response.data._status == true) {
                    setEnquiry(response.data._data)
    ...
    const getallvalue = () => {
        if (checkedValues.length === enquiry.length) {
            setCheckedValues([]);
        } else {
            setCheckedValues(enquiry.map((v) => v._id))
        }
    }
    const getvalue = (id) => {
    ...
    const changeStatus = () => {
        if (checkedValues.length > 0) {
            axios.put(import.meta.env.VITE_API_URL + import.meta.env.VITE_ENQUIRY_STATUS, {
                id: checkedValues,
            })
                .then((response) => {
                    if (response.data._status === true) {
    ...
    const deleteRecords = () => {
        if (checkedValues.length > 0) {
            if (window.confirm("Are you sure you want to delete?")) {
                ...
                axios.put(import.meta.env.VITE_API_URL + import.meta.env.VITE_ENQUIRY_DELETE, {
                    id: checkedValues,
                })
                    .then((response) => {
                        if (response.data._status === true) {
                                        ...
                                        <tr>
                                            <th scope="col" class="p-4">
                                                <div class="flex items-center">
                                                    <input
                                                        onClick={getallvalue}
                                                        checked={checkedValues.length === enquiry.length}
                                    ...
                                    <tbody className=''>

                                        {
                                            enquiry.length > 0
                                                ...
                                                enquiry.map((v, i) => {
                                                    return (
                                                        <tr class="bg-white text-center   border-gray-200 hover:bg-gray-50 ">
                                                            <td class="w-4 p-4">
                                                                <div class="flex items-center">
                                                                    <input
                                                            ...
                                                            <td class=" py-4 ">
                                                                <div className='flex items-center'> {v.email}</div>
                                                            </td>
                                                            <td class=" py-4">
                                                                <div className='flex items-center'> {v.mobile_number}</div>
                                                            </td>
                                                            <td class=" py-4">
                                                                <div className='flex items-center'>   {v.subject} </div>
                                                            </td>
                                                            <td class=" py-4">
                                                                <div className='flex items-center'>      {v.message} </div>
                                                            </td>
                                                            <td class=" py-4">
                                                                {
                                                                    v.status == 1
