...
export default function AddSubSubCategory() {
  ...
  useEffect(() => {
    if (updateId) {
      axios.post(import.meta.env.VITE_API_URL + import.meta.env.VITE_SUB_SUB_CATEGORY_DETAILS + updateId)
        .then((response) => {
          if (response.data._status == true) {
  ...
  const formHandler = (event) => {
    ...
    if (!updateId) {
      // Create category
      axios.post(import.meta.env.VITE_API_URL + import.meta.env.VITE_SUB_SUB_CATEGORY_CREATE, event.target)
        .then((response) => {
          if (response.data._status == true) {
      ...
      //Update category
      axios.put(import.meta.env.VITE_API_URL + import.meta.env.VITE_SUB_SUB_CATEGORY_UPDATE + updateId, event.target)
        .then((response) => {
          if (response.data._status == true) {
            toast.success(response.data._message);
            navigate('/category/sub-sub-category/view');
            ...
            <div className="flex gap-5">
              ...
              <div className="w-2/3">
                ...
                <div className="mb-5">
                  ...
                  <select
                    onChange={filterParentCategory}
                    value={subCategory || categoryDetails.sub_category_id || ""}  // <- this is key
