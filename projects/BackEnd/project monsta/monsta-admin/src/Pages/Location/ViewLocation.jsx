...
export default function ViewLocation() {
  ...
  useEffect(() => {
    axios.post(import.meta.env.VITE_API_URL + import.meta.env.VITE_COUNTRY_VIEW)
      .then((response) => {
        if (response.data._status == true) {
          setCountry(response.data._data)
        } else {
          setCountry([])
  ...
  const getallvalue = () => {
    if (checkedValues.length === country.length) {
      setCheckedValues([]);
    } else {
      setCheckedValues(country.map((v) => v._id))
    }
  }

  const changeStatus = () => {
    if (checkedValues.length > 0) {
      axios.put(import.meta.env.VITE_API_URL + import.meta.env.VITE_COUNTRY_STATUS, {
        id: checkedValues,
      })
        .then((response) => {
          if (response.data._status === true) {
  ...
  const deleteRecords = () => {
    if (checkedValues.length > 0) {
      if (window.confirm("Are you sure you want to delete?")) {
        ...
        axios.put(import.meta.env.VITE_API_URL + import.meta.env.VITE_COUNTRY_DELETE, {
          id: checkedValues,
        })
          .then((response) => {
            if (response.data._status === true) {
        ...
        <div className="max-w-[1220px] mx-auto py-5">
          <div className='flex item-center justify-between bg-slate-100 py-3 px-4 rounded-t-md border border-slate-400'>
            ...
            <div className='flex justify-between '>
              ...
              <button
                onClick={changeStatus} className="px-4 me-2 py-2 rounded-md text-sm font-medium bg-green-600 hover:bg-green-700 text-white transition" >
                Change Status
              </button>
                    ...
                    <tr>
                      <th scope="col" class="p-4">
                        <div class="flex items-center">
                          <input
                            onClick={getallvalue}
                            checked={checkedValues.length === country.length}
