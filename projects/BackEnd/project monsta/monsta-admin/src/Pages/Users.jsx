...
export default function Users() {
    ...
    let [userList, setUserLis] = useState([]);
    ...
    useEffect(() => {
        axios.post(import.meta.env.VITE_API_URL + import.meta.env.VITE_USERLIST_VIEW, {
            name: search,
            page: currentPage
        })
            .then((response) => {
                if (response.data._status == true) {
                    setUserLis(response.data._data);
    ...
    const getallvalue = () => {
        if (checkedValues.length === userList.length) {
            setCheckedValues([]);
        } else {
            setCheckedValues(userList.map((v) => v._id))
        }
    }

    const changeStatus = () => {
        if (checkedValues.length > 0) {
            axios.put(import.meta.env.VITE_API_URL + import.meta.env.VITE_USERLIST_STATUS, {
                id: checkedValues,
            })
                .then((response) => {
                    if (response.data._status === true) {
    ...
    const deleteRecords = () => {
        if (checkedValues.length > 0) {
            if (window.confirm("Are you sure you want to delete?")) {
                ...
                axios.put(import.meta.env.VITE_API_URL + import.meta.env.VITE_USERLIST_DELETE, {
                    id: checkedValues,
                })
                    .then((response) => {
                        if (response.data._status === true) {
    ...
    return (
        <section className="w-full">
            ...
            <div className={` rounded-lg border border-gray-300 px-5 py-5 max-w-[1220px] mx-auto mt-10 ${activeFilter ? "hidden" : "block"}`}>
                ...
                <form onSubmit={searchUser} className="flex max-w-sm">
                ...
                <div className="max-w-[1220px] mx-auto py-5">
                    <div className='flex item-center justify-between bg-slate-100 py-3 px-4 rounded-t-md border border-slate-400'>
                        ...
                        <div className='flex justify-between '>
                            <div onClick={() => setactiveFilter(!activeFilter)} className="cursor-pointer  h-[40px]  mx-3  w-[40px]  flex items-center justify-center  text-white bg-blue-700 rounded-lg border border-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                                            ...
                                            <th scope="col" class="p-4">
                                                <div class="flex items-center">
                                                    <input id="checkbox-all-search"
                                                        onClick={getallvalue}
                                                        checked={checkedValues.length === userList.length}
                                    ...
                                    <tbody>
                                        {
                                            userList.length > 0
                                                ...
                                                (userList.map((v, i) => {
                                                    return (
                                                        <tr class="bg-white   border-gray-200 hover:bg-gray-50 ">
                                                            <td class="w-4 p-4">
                                                                <div class="flex items-center">
                                                                    <input
