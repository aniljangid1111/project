...
export default function ProductItems() {
  ...
  const [showModal, setShowModal] = useState(false); //image big
  // pagination work
  const [currentPage, setCurrentPage] = useState(1);
  const [totalPages, setTotalPages] = useState(1);
  // filter option parent cartegory
  const [parentCategory, setParentCategory] = useState('');
  const [category, setCategory] = useState([]);
  // filter option sub categories
  const [subCategory, setSubCategory] = useState('');
  const [subItem, setSubItem] = useState([]);
  // filter option subsub categories
  ...
  useEffect(() => {
    axios
      ...
      .then((response) => {
        if (response.data._status === true) {
          setSubSubItems(response.data._data);
        } else {
          setSubSubItems([])
          // toast.error(response.data._message);
        }
  ...
  useEffect(() => {
    axios
      .post(import.meta.env.VITE_API_URL + import.meta.env.VITE_PRODUCTS_VIEW, {
        name: searchName,
        page: currentPage,
        parent_category_ids: parentCategory,
        sub_category_ids: subCategory,
        sub_sub_category_ids: subSubCategory,
  ...
  }, [searchName, apiStatus, currentPage, parentCategory, subCategory, subSubCategory]);
  ...
  const filterParentCategory = (event) => {
    ...
    } else if (name === 'subsubCatSelectBox') {
  ...
  // }
  // const filtersubsubcategory = (event) => {
  ...
  const getAllValues = () => {
    if (checkedValues.length === products.length) {
      ...
      setCheckedValues(products.map((v) => v._id));
    }
  };
  const changeStatus = () => {
    if (checkedValues.length > 0) {
      axios
        .put(import.meta.env.VITE_API_URL + import.meta.env.VITE_PRODUCTS_STATUS, {
          id: checkedValues,
        })
        .then((response) => {
          if (response.data._status === true) {
  ...
  const deleteRecords = () => {
    if (checkedValues.length > 0) {
      if (window.confirm("Are you sure you want to delete?")) {
        axios
          .put(import.meta.env.VITE_API_URL + import.meta.env.VITE_PRODUCTS_DELETE, {
            id: checkedValues,
          })
          .then((response) => {
            if (response.data._status === true) {
  ...
  return (
    <section className="w-full">
      ...
      <AnimatePresence>
        {!activeFilter && (
          <motion.div
            ...
            animate="visible"
            exit="exit"
            className="border border-gray-300 px-5 py-5 max-w-[1220px] mx-auto mt-10 rounded-lg bg-white shadow"
          >
            <form className="grid grid-cols-[20%__20%_20%] gap-[1%] items-center ">
              <div className="">
      ...
      <div
        className={`
    max-w-[1220px] mx-auto mt-10 transition-all duration-500 ease-in-out transform origin-top 
    ${activeFilter ? "translate-y-[-10px] opacity-0 max-h-0 overflow-hidden" : "translate-y-0 opacity-100 max-h-[500px]"}
    border border-gray-300 px-5 py-5 rounded-lg
  `}
      >
          ...
          <div className="border border-t-0 rounded-b-md border-slate-400">
            <div className="relative overflow-x-auto">
              <table className="w-full  text-center text-gray-500 ">
                <thead className="text-[13px] text-gray-700 uppercase bg-gray-50 ">
                  <tr>
                    <th scope="col" className="p-4">
                      <div className="flex items-center">
                        <input
                          onChange={getAllValues}
                          checked={checkedValues.length === products.length}
                        ...
                        return (
                          <tr key={index} className="bg-white border-b">
                            ...
                            <td scope="row" className=" px-6 py-4 text-gray-900 ">
                              <div className="py-4  ">
                                {
                                  value.parent_category_ids != '' && value.parent_category_ids.length > 0
                                    ?
