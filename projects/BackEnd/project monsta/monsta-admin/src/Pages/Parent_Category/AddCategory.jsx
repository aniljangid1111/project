...
export default function AddCategory() {
  ...
  useEffect(() => {
    if (updateId) {
      axios.post(import.meta.env.VITE_API_URL + import.meta.env.VITE_CATEGORY_DETAILS + updateId)
        .then((response) => {
  ...
  const formHandler = (event) => {
    ...
    if (!updateId) {
      // Create category
      axios.post(import.meta.env.VITE_API_URL + import.meta.env.VITE_CATEGORY_CREATE, event.target)
        .then((response) => {
          if (response.data._status == true) {
      ...
      //Update category
      axios.put(import.meta.env.VITE_API_URL + import.meta.env.VITE_CATEGORY_UPDATE + updateId, event.target)
        .then((response) => {
          if (response.data._status == true) {
            toast.success(response.data._message);
            navigate('/category/view');
