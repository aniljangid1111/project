...
export default function AddCategory() {
  ...
  const formHandler = (event) => {
    ...
    if (event.target._id.value == "") {
      axios
        .post("http://localhost:8400/api/admin/category/add", event.target)
        .then((success) => {
      ...
      axios
        .put(
          `http://localhost:8400/api/admin/category/update/${event.target._id.value}`,
          event.target
        )
        .then((success) => {
          if (success.data.status == true) {
  ...
  useEffect(() => {
    if (params.id != null) {
      axios
        .post(`http://localhost:8400/api/admin/category/details/${params.id}`)
        .then((result) => {
